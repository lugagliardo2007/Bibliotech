...
## 🆕 Registro de cuentas para docentes

Ahora los docentes pueden crear su propia cuenta desde la pantalla de
login ("¿Sos docente y no tenés cuenta? Creá una acá"), con nombre,
email y contraseña. Esto agrega:

- `api/registrar.js` — crea el usuario con `Rol = 'docente'` (el
  registro público es solo para docentes; los bibliotecarios se siguen
  cargando a mano en la base).
- Contraseñas hasheadas con `bcrypt` para las cuentas nuevas — ya no
  quedan en texto plano. `api/login.js` ahora detecta si la contraseña
  guardada es un hash (`$2...`) o texto plano (cuentas viejas) y
  compara de la forma que corresponda, así no rompés el login de las
  cuentas que ya tenías cargadas.
- El login ahora acepta usuario **o email** en el mismo campo.

### ⚠️ Paso obligatorio: agregar la columna Email

Tu tabla `Usuario` no tiene columna de email todavía. Antes de
desplegar esto, entrá a Neon → tu proyecto → **SQL Editor** y corré:

```sql
ALTER TABLE "Usuario" ADD COLUMN IF NOT EXISTS "Email" text;
CREATE UNIQUE INDEX IF NOT EXISTS usuario_email_unique
  ON "Usuario" ("Email") WHERE "Email" IS NOT NULL;
```

Esto agrega la columna sin tocar los usuarios que ya existen (les
queda `Email = NULL`, así que van a seguir entrando con su
`Nombre` de siempre).
